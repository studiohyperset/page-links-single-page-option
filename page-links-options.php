...

{
...

    {
		...
		wp_register_script('admin-menu-current', plugins_url('js/admin-menu-current.js', __FILE__),array('jquery'));	
...

    {
        ...
        if ($option['type'] == 'multicheckcp') {
			...
			$cps =  get_post_types(array('public' => true, '_builtin' => false));
			...
			foreach ($cps as $cp ) {
				...
				if (is_array($value))
					...
					if (in_array($cp, $value)) $checked = "CHECKED ";

				else

					if ($cp == $value) $checked = "CHECKED ";

				

				$option_str .= "<label for=\"{$option['id']}_$i\">"
